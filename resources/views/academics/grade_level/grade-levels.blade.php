...
<div class="px-5 py-3 flex flex-col gap-4">
   <div class="breadcrumbs text-xs">
      <ul>
         <li><a>Academics</a></li>
         <li class="text-blue-600 font-semibold">Grade Levels</li>
      </ul>
   </div>

   <div class="rounded-lg bg-blue-600 shadow-lg">
      <h1 class="text-[24px] font-semibold text-base-300 ms-3 p-2">Grade Levels</h1>
   </div>
   ...
   <div class="overflow-x-auto rounded-lg">
      <table class="table">
         ...
         <tbody>
            ...
            <tr class="hover:bg-base-300">
               ...
               <td>
                  <span
                     class="{{ $gradeLevel->is_active ? 'badge badge-soft badge-success badge-sm' : 'badge badge-soft badge-neutral badge-sm' }}">
               ...
               <td class="w-25">
                  <div class="flex gap-2">
                     <a href="{{route('grade-levels.edit', $gradeLevel->id)}}"
                        class="btn btn-soft px-1 text-blue-600 btn-xs tooltip hover:bg-blue-700 hover:text-base-300 rounded-lg"
                        data-tip="Edit Details">
                        <i class="fi fi-sr-pen-square text-[18px] pt-1"></i>
                     </a>

                     <button
                        class="btn btn-soft px-1 text-blue-600 btn-xs tooltip hover:bg-blue-700 hover:text-white rounded-lg"
                        data-tip="Delete"
            ...
            <dialog id="delete_modal_{{ $gradeLevel->id }}" class="modal">
               <div class="modal-box">
            ...
            <tr>
               <td colspan="5" class="text-center">
                  <div class="flex flex-col items-center gap-2 py-8">
                     <i class="fi fi-sr-book text-4xl text-gray-400"></i>
                     <div class="text-gray-500">
                        @if(request('search'))
                        <p class="font-semibold">No grade levels found matching "{{ request('search') }}"</p>
                        <p class="text-sm mt-1">Try adjusting your search terms</p>
                        @else
                        <p class="font-semibold">No grade levels available</p>
                        <p class="text-sm mt-1">Create a new grade level to get started</p>
                        @endif
                     </div>
                  </div>
               </td>
      ...
      <div class="flex justify-center items-center gap-3 p-4">
         <div>
            {{ $gradeLevels->appends(['search' => request('search')])->links('vendor.pagination') }}
         </div>
      </div>
      @endif
   </div>
