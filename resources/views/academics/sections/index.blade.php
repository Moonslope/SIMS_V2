   ...
   <div class="card bg-base-100 shadow-md">
      <div class="card-body p-4">
         <div class="flex flex-col sm:flex-row justify-between items-center gap-3">
            <form action="{{ route('sections.index') }}" method="GET" class="flex flex-col sm:flex-row gap-2 items-center w-full sm:w-auto" id="searchForm">
               <label class="input input-sm input-bordered flex items-center gap-2 w-full sm:w-80">
                  ...
                  @if(request('search'))
                  <a href="{{ route('sections.index', ['academic_year' => request('academic_year'), 'grade_level' => request('grade_level')]) }}" class="btn btn-xs btn-circle btn-ghost" title="Clear search">
               ...
               <select name="academic_year" class="select select-sm select-bordered w-full sm:w-48" onchange="this.form.submit()">
