...
<div class="px-5 py-3 flex flex-col gap-4">
   <div class="breadcrumbs text-xs">
      <ul>
         <li><a>Academic</a></li>
         <li><a>Program Types</a></li>
      </ul>
   </div>

   <div class="rounded-lg bg-primary shadow-lg">
      <h1 class="text-[24px] font-semibold text-base-300 ms-3 p-2">Program Types</h1>
   </div>
   ...
   <div class="card bg-base-100 shadow-md">
      <div class="card-body p-4">
         <div class="flex flex-col sm:flex-row justify-between items-center gap-3">
            <form action="{{ route('program-types.index') }}" method="GET" id="searchForm" class="w-full sm:w-auto">
               <label class="input input-sm input-bordered flex items-center gap-2 w-full sm:w-80">
                  ...
                  <input type="search" name="search" id="searchInput" value="{{ request('search') }}"
                     placeholder="Search program name..." class="grow focus:outline-none" />
                  @if(request('search'))
                  <a href="{{ route('program-types.index') }}" class="btn btn-xs btn-circle btn-ghost"
                     title="Clear search">
            ...
            <a href="{{route('program-types.create')}}"
   ...
   <div class="overflow-x-auto rounded-lg">
      <table class="table">
         ...
         <tbody>
            ...
            <tr class="hover:bg-base-300">
               ...
               <td>
                  <span
                     class="{{ $programType->is_active ? 'badge badge-soft badge-success badge-sm' : 'badge badge-soft badge-neutral badge-sm' }}">
                     {{ $programType->is_active ? 'Active' : 'Inactive' }}
                  </span>
               </td>
               <td class="w-25">
                  <div class="flex gap-2">
                     <a href="{{route('program-types.edit', $programType->id)}}"
                     ...
                     <button
                        class="btn btn-soft px-1 text-primary bg-primary-content btn-xs tooltip hover:bg-primary hover:text-white"
                        data-tip="Delete"
                        onclick="document.getElementById('delete_modal_{{ $programType->id }}').showModal()">
                        <i class="fi fi-sr-trash text-[18px] pt-1"></i>
                     </button>
                  </div>
               </td>
            </tr>
            <dialog id="delete_modal_{{ $programType->id }}" class="modal">
               <div class="modal-box">
                  ...
                  <div class="modal-action flex justify-center">
                     <div class="flex gap-2">
                        ...
                        <form action="{{ route('program-types.destroy', $programType->id) }}" method="POST">
                           @csrf
                           @method('DELETE')
            ...
            <tr>
               <td colspan="5" class="text-center">
                  <div class="flex flex-col items-center gap-2 py-8">
                     <i class="fi fi-sr-diploma text-4xl text-gray-400"></i>
                     <div class="text-gray-500">
                        @if(request('search'))
                        <p class="font-semibold">No program types found matching "{{ request('search') }}"</p>
                        <p class="text-sm mt-1">Try adjusting your search terms</p>
                        @else
                        <p class="font-semibold">No program types available</p>
                        <p class="text-sm mt-1">Create a new program type to get started</p>
                        @endif
                     </div>
                  </div>
               </td>
      ...
      @if($programTypes->hasPages())
      <div class="flex justify-center items-center gap-3 p-4">
         <div>
            {{ $programTypes->appends(['search' => request('search')])->links('vendor.pagination') }}
         </div>
      </div>
      @endif
   </div>
