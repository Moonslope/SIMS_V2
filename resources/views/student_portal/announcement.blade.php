...
<div class="px-5">
   <div class="my-12">
      <h1 class="text-4xl font-bold text-[#0F00CD] mb-2 text-center">Announcements</h1>
      ...
      <div class="card bg-base-100 shadow-md hover:shadow-lg transition-shadow">
         <div class="card-body">
            ...
            <div class="flex justify-between items-start gap-4">
               <div class="flex-1">
                  <h2 class="card-title text-[#0F00CD] mb-2">{{ $announcement->title }}</h2>
                  <div class="flex flex-col sm:flex-row gap-2 sm:gap-4 text-sm text-gray-600">
                     <div class="flex items-center gap-1">
                        <i class="fi fi-sr-calendar text-[#0F00CD]"></i>
                        <span>{{ $announcement->announcement_date->format('M d, Y') }}</span>
                     </div>
                     <div class="flex items-center gap-1">
                        <i class="fi fi-sr-user text-[#0F00CD]"></i>
            ...
            <div class="card-actions justify-end mt-4">
               <button class="btn bg-[#0F00CD] btn-sm text-white"
      ...
      <dialog id="announcement_modal_{{ $announcement->id }}" class="modal">
         <div class="modal-box w-11/12 max-w-2xl max-h-[90vh] overflow-y-auto">
            <div class="flex justify-between items-start mb-4">
               <h3 class="font-bold text-lg text-[#0F00CD]">{{ $announcement->title }}</h3>
               <form method="dialog">
