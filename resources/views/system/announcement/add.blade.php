...
<div class="px-5 py-3 flex flex-col gap-4">
   <div class="breadcrumbs text-xs">
      <ul>
         ...
         <li><a>Create New Announcement</a></li>
      </ul>
   </div>

   <div class="rounded-lg bg-blue-600 shadow-lg">
      <h1 class="text-[24px] font-semibold text-base-300 ms-3 p-2">Create New Announcement</h1>
   </div>
         ...
         <div class="flex flex-col gap-5">
            <div class="grid grid-cols-2 gap-5">
               <div class="flex flex-col gap-2">
                  ...
                  <input name="title" type="text" placeholder="e.g., School Picnic Announcement"
                     class="rounded-lg w-full input @error('title') input-error @enderror" value="{{ old('title') }}" />
                  @error('title')
               ...
               <div class="flex flex-col gap-2">
                  ...
                  <input id="announcement_date" name="announcement_date" type="date"
                     class="rounded-lg w-full input @error('announcement_date') input-error @enderror"
                     value="{{ old('announcement_date', now()->format('Y-m-d')) }}" />
            ...
            <div class="flex justify-end">
               <div class="flex gap-3">
                  ...
                  <button type="submit" class="btn bg-blue-600 hover:bg-blue-700 text-white btn-sm w-40 rounded-lg">Save</button>
               </div>
            </div>
         </div>
      </form>
